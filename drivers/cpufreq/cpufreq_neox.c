...
struct workqueue_struct *dvfs_workqueue2;
...
static void apply_hotplug_lock(void)
{
	...
	queue_work_on(dbs_info->cpu, dvfs_workqueue2, work);
}

int cpufreq_pegasusq_cpu_lock2(int num_core)
{
	int prev_lock;
...
int cpufreq_pegasusq_cpu_unlock2(int num_core)
{
...
void cpufreq_neox_min_cpu_lock(unsigned int num_core)
{
	int online, flag;
	struct cpu_dbs_info_s *dbs_info;
	...
	if (flag <= 0)
		return;
	queue_work_on(dbs_info->cpu, dvfs_workqueue2, &dbs_info->up_work);
}

void cpufreq_neox_min_cpu_unlock(void)
{
	...
	if (flag >= 0)
		return;
	queue_work_on(dbs_info->cpu, dvfs_workqueue2, &dbs_info->down_work);
}
...
struct cpu_usage_history *hotplug_history2;
				  ...
				  const char *buf, size_t count)
{
	...
	if (ret != 1)
		return -EINVAL;
	if (input == 0)
		cpufreq_neox_min_cpu_unlock();
	else
		cpufreq_neox_min_cpu_lock(input);
	return count;
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (prev_lock)
		cpufreq_pegasusq_cpu_unlock2(prev_lock);
	...
	ret = cpufreq_pegasusq_cpu_lock2(input);
	if (ret) {
...
{
	int num_hist = hotplug_history2->num_hist;
	struct cpu_usage *usage;
	int freq, rq_avg;
	int avg_load;
	int i;
	...
	for (i = num_hist - 1; i >= num_hist - up_rate; --i) {
		usage = &hotplug_history2->usage[i];
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if(hp_s_delay > 0 && hp_s_delay != hp_s_delayc) {
		hotplug_history2->usage[num_hist].freq = 
			(hotplug_history2->usage[num_hist].freq * 
	...
	hotplug_history2->usage[num_hist].freq = policy->cur;
	
#ifdef CONFIG_CPU_FREQ_GOV_ONDEMAND_FLEXRATE
	if(hp_s_delay <= 1){
#endif
		
	//hotplug_history2->usage[num_hist].rq_avg = avg_nr_running();
	++hotplug_history2->num_hist;
...
#ifdef CONFIG_CPU_FREQ_GOV_ONDEMAND_FLEXRATE
		if(hp_s_delay > 0 && hp_s_delay != hp_s_delayc)
		  hotplug_history2->usage[num_hist].load[j] = 
			(hotplug_history2->usage[num_hist].load[j] * 
			(hp_s_delayc - hp_s_delay) + load) / 
			(hp_s_delayc - hp_s_delay + 1);
		else
#endif
		hotplug_history2->usage[num_hist].load[j] = load;
...
#ifdef CONFIG_CPU_FREQ_GOV_ONDEMAND_FLEXRATE
	if(hp_s_delay > 0 && hp_s_delay != hp_s_delayc)
		hotplug_history2->usage[num_hist].avg_load = 
		(hotplug_history2->usage[num_hist].avg_load * 
		(hp_s_delayc - hp_s_delay) + avg_load) / 
		(hp_s_delayc - hp_s_delay + 1);
	else
#endif
	hotplug_history2->usage[num_hist].avg_load = avg_load;
	...
	if (check_up()) {
		queue_work_on(this_dbs_info->cpu, dvfs_workqueue2,
			      &this_dbs_info->up_work);
	} else if (check_down()) {
		queue_work_on(this_dbs_info->cpu, dvfs_workqueue2,
			      &this_dbs_info->down_work);
	}
	if (hotplug_history2->num_hist  == max_hotplug_rate)
		hotplug_history2->num_hist = 0;
#ifdef CONFIG_CPU_FREQ_GOV_ONDEMAND_FLEXRATE
	}
#endif
	...
	if(hp_s_delay > 0 && hp_s_delay != hp_s_delayc)
		...
		for_each_cpu(j, policy->cpus) {
			...
			load_freq = hotplug_history2->usage[num_hist].load[j] * 
				    hotplug_history2->usage[num_hist].freq;
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, dvfs_workqueue2, &dbs_info->work, delay);
	mutex_unlock(&dbs_info->timer_mutex);
}

#ifdef CONFIG_CPU_FREQ_GOV_ONDEMAND_FLEXRATE
int cpufreq_ondemand_flexrate_request2(unsigned int rate_us, unsigned int duration)
{
...
EXPORT_SYMBOL_GPL(cpufreq_ondemand_flexrate_request2);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, dvfs_workqueue2,
...
unsigned int prev_freq_step2;
unsigned int prev_sampling_rate2;
#ifdef CONFIG_CPU_FREQ_LCD_FREQ_DFS
...
static void cpufreq_neox_early_suspend(struct early_suspend *h)
{
	...
	dbs_tuners_ins.lcdfreq_enable = false;
#endif
	prev_freq_step2 = dbs_tuners_ins.freq_step;
	prev_sampling_rate2 = dbs_tuners_ins.sampling_rate;
...
static void cpufreq_neox_late_resume(struct early_suspend *h)
{
	...
	dbs_tuners_ins.freq_step = prev_freq_step2;
	dbs_tuners_ins.sampling_rate = prev_sampling_rate2;
#if EARLYSUSPEND_HOTPLUGLOCK
	apply_hotplug_lock();
#endif
}
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	hotplug_history2 = kzalloc(sizeof(struct cpu_usage_history), GFP_KERNEL);
	if (!hotplug_history2) {
	...
	dvfs_workqueue2 = create_workqueue("kneox");
	if (!dvfs_workqueue2) {
...
err_reg:
	destroy_workqueue(dvfs_workqueue2);
err_queue:
	kfree(hotplug_history2);
err_hist:
	return ret;
}
